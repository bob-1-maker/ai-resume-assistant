...
const projectCurrentStatus = ref<Record<string, boolean>>({})
...
const syncProjects = () => {
  if (resumeStore.currentResume) {
    ...
    projects.value.forEach(project => {
      if (projectCurrentStatus.value[project.id] === undefined) {
        projectCurrentStatus.value[project.id] = project.endDate === '至今'
      }
    })
  }
}
...
const addProject = () => {
  ...
  projectCurrentStatus.value[newProject.id] = false
  saveProjects()
...
const removeProject = (id: string) => {
  ...
  if (index >= 0) {
    ...
    delete projectCurrentStatus.value[id]
    saveProjects()
...
const toggleCurrent = (id: string) => {
  const project = projects.value.find(project => project.id === id)
  if (project) {
    projectCurrentStatus.value[id] = !projectCurrentStatus.value[id]
    if (projectCurrentStatus.value[id]) {
      project.endDate = '至今'
    } else {
      project.endDate = ''
    }
              ...
              <el-form-item label="结束时间" required>
                <div class="end-date-container">
                  <el-date-picker 
                    ...
                    value-format="YYYY-M"
                    :picker-options="{ months: ['一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月'] }"
                    :disabled="projectCurrentStatus[project.id]"
                    @change="updateProject(project.id, 'endDate', project.endDate)"
                  />
                  <el-checkbox 
                    :checked="projectCurrentStatus[project.id]" 
                    @change="toggleCurrent(project.id)"
